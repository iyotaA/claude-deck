/* 画面側の状態と、その置き場所。
 *
 * いま持っている状態（store）と、URL への書き戻し（syncQuery）。
 * 状態を1箇所に集めておくと、どこが何を書き換えているかを追える。
 *
 * **画面の節点（dom）は `dom.js` に分けてある。** 持っているものの性質が違う
 * ―― あちらは起動時に決まって二度と変わらない参照の表、こちらは動くたびに書き換わる状態。
 * この塊が同居していたころ、`store.js` の 24% が節点の索引だった。
 *
 ...
 * 数値（横断）の絞り込みの既定。
 *
 * **1箇所に置く。** 起動時の値と、「絞り込みを外す」で戻る先が同じものなので、
 * 2箇所に書くと片方だけ直した日に「外したのに既定へ戻らない」になる。
 * 下の `usageTab` はここを展開して初期値にしている。
 *
 * 書庫の「**絞り込みを外すときは並び順も戻す**」（外したのに大きい順のままだと
 * 外れていないように見える）と同じ話で、あちらは戻す先が `ARCHIVE_SORTS` の
 * 先頭という約束だった。こちらは値が3つあるので表にしてある。
 */
