...
| 3 | `run-view.js` | 実行のパネル（詳細ペイン側）。器を預かって自分で描き直す。入力欄と替える窓は**ペインの外**に持つ |
...
| `run.css` | 実行のパネル（速報の行・種類の札・入力欄・いま何で走っているかの札）と、起こすフォーム・替える窓の器 |
...
- **未応答の許可要求を速報（`events`）から読まない。** あちらは `EVENTS_PER_RUN` が 400 で、1ターンで数百行来る。溢れると**要求そのものが消えて二度と答えられない。** 行（`row.asks`）から読む。`rows` は毎フレーム総入れ替えなので、取りこぼしが次のフレームで自己修復する
...
- **替える口を実行パネルの中に置かない。** 速報が数百行伸びるので、パネルの中に置くと**いちばん替えたい場面（走っている最中）に限って画面の下へ流れて見つからない**（実測。0.7.1 の `<details>` がそうなっていた）。いまは入力欄のすぐ上の札（`.composer-now`）が入り口で、中身は `document.body` に付けた `<dialog class="runsw">`。**どちらも詳細ペインの外**なので作り直しに巻き込まれない
