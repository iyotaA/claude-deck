...
- **枠の使用率をセッションごとに出さない。** アカウント共通の値なので、実行の数だけ同じ数が並ぶうえ「この実行が使った枠」だと読める。出すのは上のバーに1つだけ（`list.js` の `renderRate`）。届く道が実行の stdout（`rate_limit_event`）しか無いので**行に載って来る**が、それは出どころの都合であって「その実行の枠」という意味ではない
