# 読み取り層

> `src/read/` を触るときに読む。
> 全体の地図と横断の約束はルートの `CLAUDE.md` にある。
> 一覧と詳細で経路が分かれている話（性能設計の中心）はルートの「データの流れ」にある。

`~/.claude` 配下を **読み取り専用** で解析する層。読み取り元は `src/read/paths.mjs` が解決し、
`CLAUDE_CONFIG_DIR` があればそちらを優先する。

| 場所 | 中身 |
|---|---|
| `sessions/<PID>.json` | 稼働中セッションの登録簿 |
| `projects/<スラッグ化した cwd>/<セッションID>.jsonl` | 会話ログ |
| `tasks/<セッションID>/<番号>.json` | TODO の状態 |

`read/cache.mjs` がサイズと mtime を印にしてパース結果を memo する。
会話ログは追記しか起きないので、印が同じなら前回の値を使い回せる。
一覧を毎秒引いても重くならないのはこの土台のおかげ。

ディスクを触る側（`listSubagents` / `readPlanFile` / `readHead` / `listArchive`）にはテストが無い。
`configDir` が import 時に一度だけ評価される定数なので、差し替えるには import 順の細工が要る。
代わりに**判断（純関数）と I/O（薄い殻）を分ける形**にして、判断だけをテストできるようにしてある。
ここに残っているのは「readdir して、名前で絞って、stat して、try/catch で飲む」だけ。

## 本文の検索（`grep.mjs`）

**索引を持たない。そのつど舐める。**

スキルの索引（`skills.mjs`）と同じ形にもできるが、あちらは「いつ作り直すか」を
抱えている（起動時に1回きりで、起動後に生まれたセッションが絞り込みに出ない、
という不具合を実際に踏んだ）。本文の索引はスキル名より遥かに大きく、
鮮度の問題も同じだけ抱える。**逐次で舐めれば、いまディスクにあるものをそのまま見る。**

速さの根拠は `skills.mjs` と同じ。全行を `JSON.parse` せず、**検索語そのものを
前フィルタに使う**。会話ログの日本語は**生の UTF-8 で入っている**（エスケープ表記ではない。実測）
ので、素の `includes` がそのまま効く。**実測 155 MB/s**（14.1MB・4本を 90ms）。

読むのは3つだけ ―― ユーザーの指示・Claude の言葉・プランの本文。
`toolUseResult` は数MBになることがあり、そこに当たっても「どこで当たったか」の役に立たない。

**当たった行だけが `JSON.parse` まで届く。** 前フィルタは行そのものを見ているので、
`uuid` や置き場所のパスに当たっただけの行も来る ―― 本文に無ければ数えない。

遅ければあとから索引を足せる。窓口の形（`hits` を行に載せる）を先に決めてあるので、
中身を差し替えても画面は触らずに済む。

## 触るときに壊してはいけないこと（読み取り）

- **生死の判定は PID の存在確認だけ。** 登録簿の `updatedAt` は状態が変わったときしか書かれない。古さで終了扱いにすると稼働中のセッションが一覧から消える
- **`slugifyCwd` の結果からパスを復元しない。** 英数字以外がすべて `-` になる不可逆変換。cwd は登録簿とログの各行から直接取る
- **`indexTranscripts` を再帰にしない。** 今は2階層固定なので `<セッションID>/subagents/agent-*.jsonl` を弾けている。再帰にすると 148 ファイルが「セッション」として一覧に流れ込む
- **`listSubagents` に memo を掛けない。** 印に使えるのはディレクトリの mtime だが、NTFS はファイルが増えたときは変わるのに、既存ファイルが太ったときは変わらない。走っているサブエージェントの大きさが固まって表示が伸びなくなる
- **サブエージェントの `agentId` をパスに連結しない。** 開くファイルは常に `readdir` が返した名前にする。だからパストラバーサルの検証が要らない。`server.mjs` の正規表現は入口の粗いふるいであって、安全の根拠ではない
- **プランのファイルは `plansDir` の中を指すことを確かめてから開く。** `filePath` はログ由来の外部入力。比較は `path.relative` で行う（`startsWith` はドライブレターの大小で正しいパスを弾く）
- **プランの mtime だけで「提出後に変わった」と断定しない。** 実測45件で、ファイルのほうが古い例が28件あった。本文の一致を主判定にする。`planWasEdited` はキーが無いことを「編集なし」と読み替えない
- **一覧の経路（毎秒走る）に重い処理を足さない。** ここに何かを足すときは、まず「払わずに済む行」を先に落とす。サブエージェントの件数がその形で、`indexTranscripts` が `withFileTypes` でタダで拾った `hasSessionDir` を見て、`<セッションID>/` が無い行は `readdir` を1回も出さずに 0 と決める。実測 168 件中 48 件しか持っていない。全件を数えると `indexTranscripts` が 39.0ms → 78.2ms になり、一覧全体（48.8ms）を超える
- **件数を数えるのに `listSubagents` を呼ばない。** 一覧は `countSubagents`（`readdir` 1回だけ）。`listSubagents` は 1件ごとに `stat` と `.meta.json` を読むので、詳細を開いたときだけ

## スキルの索引（`skills.mjs`）

**この層で唯一、ディスクへ書く。** 置き場所は `%LOCALAPPDATA%\ClaudeDeck\skills.json`
（`shared/appdata.mjs` が決める）。`~/.claude` には書かない。

なぜ索引が要るか。**書庫の検索（末尾 64KB）にはスキルが1件も写らない。**
スキルは作業の入口で呼ぶのでログの先頭に出るが、ログは 1MB から 15MB あり、
末尾 64KB はその 0.4% にも届かない。実測（2026-09-04・新しい 60 本）で
**スキルを使っていた 10 本のうち、末尾から拾えたのは 0 本**だった。
「中身も探す」を押しても読む範囲は同じなので変わらない。

全文は読むが `JSON.parse` を全行に掛けない。`"Skill"` を含む行だけ解く。
実測 1 MB あたり 10 ms、**全 445 本 / 614 MB を1回舐めて約 6.1 秒**。

- **立ち上げの裏で1回だけ走らせる**（`server.mjs` の `listening`）。**待たない。**
  ここで待つと画面が出るまで固まる
- 2回目以降は大きさと更新時刻が変わったものだけ読み直す（実測で 2 本 / 1 秒）
- **`skillIndex()` は作りに行かない。** まだ無ければ空を返す。
  ここで待たせると、立ち上げ直後の1回だけ 6 秒かかる窓口ができる
- **索引が無いときに `skill=` で絞ったら 0 件を返す。** 全件に化けさせない
  （絞ったつもりで全部出ると、選んだ覚えのないものを見ることになる）
- できているかどうかは `/api/health` と `/api/archive` の `meta.skillIndex` に出す。
  画面はそれを見て札を「作成中」にする（**黙って空の候補を出さない**）
