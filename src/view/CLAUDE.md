...
**一度検討して、見送った（2026-09-06）。理由は費用。**
要約を AI に作らせると、セッションを開くたびに従量課金が発生する。
このアプリの他の機能は全部ローカルで完結していて、**開くだけで金が減るものが1つも無い。**
そこへ1つ混ぜると性質が変わる。技術的な障害があったわけではない（骨組みはそのまま貼れる）ので、
入り口は残してある。
