 ...
 *
 * ── 一度検討して、見送った（2026-09-06）──────────────────────
 *
 * **理由は費用。** 要約を AI に作らせると、セッションを開くたびに従量課金が発生する。
 * このアプリの他の機能は全部ローカルで完結していて、**開くだけで金が減るものが
 * 1つも無い**。そこへ1つ混ぜると、性質が変わる。
 *
 * 技術的な障害があったわけではない（下の骨組みはそのまま貼れる）。
 * 決まっていなかったのは送り先・鍵の置き場所・送る中身の3つで、
 * どれも決めれば済む話だった。**やらないと決めたのは、価値と費用の釣り合いのほう。**
 *
 * だから下の入り口はそのまま残してある。要ると思ったときに読む。
 *
 * ── AI 要約に差し替えるときの入り口 ──────────────────────────
 *
