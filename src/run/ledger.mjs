 ...
 * `tick()` の無音判定の射程も広げない（許可待ちの無音は正常）。
 * **時間で断ることもしない**（`PERMISSION_TIMEOUT_MS` の既定は 0 ＝ 上限なし）。
 * CLI 本体が時間で質問を閉じないので、ここだけが勝手に断ると
 * 別の作業から戻ってきたときに同じ指示をやり直すことになる。畳む口は人が持つ。
 *
 ...
 * ここが見てよいのは同じ層の `run/event.mjs` と `run/ask.mjs`、
 * どこからでも使える `shared/`、それに `parse/` の純関数（`stream.mjs` の `sameSessionId`）だけ。
 ...
 * | 置き場所 | 何を持つか |
 * |---|---|
 * | ここ | いつ状態が変わるか（状態機械・速報リング・停止と切り替えの段取り） |
 * | `run/ask.mjs` | 許可要求カードをどう見せるか（**状態を1バイトも触らない純粋な変換**） |
 * | `run/merge.mjs` | 起こした実行を一覧へどう混ぜるか（**データの向きが逆**。外から来た行を加工する） |
 *
 * `merge.mjs` はここを import するが、逆は無い。合流を呼ぶのは `server.mjs` だけ。
 */
...
import {
  ASK_BODY_MAX, askBody, askQuestions, askRow, askWhat, buildQuestionInput, suggestModeOf,
} from './ask.mjs';
...
/**
 * 許可の答えを待つ上限。**既定は 0 ＝ 上限なし（答えるまで待つ）。**
 *
 * 長く 10分（`STALL_MS` より十分長く、Slack で気づいて戻ってこられる長さ）にしてあり、
 * 「ブラウザを閉じた・席を外したときの唯一の逃げ道なので消さない」と書いてあった。
 * **その前提のほうが実態と合わなかった。**
 *
 * 逃げ道は他にもある ―― 画面から断る・止める・割り込む、のどれでも畳める。
 * 一方で自動で断る側の代償は重い。**CLI 本体は時間で質問を閉じない**ので、
 * この画面だけが 10 分で勝手に断ることになり、別の作業をしているあいだに
 * 断られていて、戻ってきたら同じことをもう一度指示する羽目になる（実運用での指摘）。
 * 「答えを待っている」は人の都合で何時間にもなる状態で、機械が測って良い長さではない。
 *
 * **0 は「切る」。** 掃く側（`sweepAsks`）が 0 以下なら何もしない。
 * `0` を素朴にしきい値として使うと `quiet >= 0` が常に真になり、
 * **来た瞬間に全部断る**ことになるので、早期 return は消さない。
 *
 * 上限が要る人（共用の機械に置いている、など）は
 * `createRunLedger({ permissionTimeoutMs })` で入れられる。設定から渡す口は
 * `run/index.mjs` にある。
 */
export const PERMISSION_TIMEOUT_MS = 0;
...
export function createRunLedger({
  ...
  function sweepAsks(run, now) {
    // **上限を切ってあるなら何もしない（既定）。**
    // ここを外すと `quiet >= 0` が常に真になり、来た瞬間に全部断ることになる
    if (!(permissionTimeoutMs > 0)) return [];
