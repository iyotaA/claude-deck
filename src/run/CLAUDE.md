...
**速報で配らない。** `EVENTS_PER_RUN = 400` で溢れる器に置くと、行が消えたときに
画面が「切り替え中」のまま取り残される。許可要求を行に載せているのと同じ理由。
...
- **切り替え中（`switching`）を速報で配らない。** 溢れると画面が「切り替え中」のまま取り残される。行に載せて、消す権利をサーバーだけが持つ
