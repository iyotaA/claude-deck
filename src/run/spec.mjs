...
/**
 * 予算の既定（USD）。**画面の欄に最初から入る値**でもある。`null` は上限なし。
 *
 * **長く 20 を入れていたが、既定は「上限なし」に変えた。**
 * 20 にした理由（実プロジェクトで 13往復・$6.52 かかって予算切れになり、
 * `--resume` で起こし直したほうも $6.09 で同じ死に方をした。合計 $12.6 使って
 * 1つも仕事が終わらないので実測の3倍にした）は今も事実だが、
 * **予算切れで途中で死ぬほうが高くつく**、というのがそのあとの実感だった。
 * 半端に終わった仕事はやり直しになり、やり直しにもまた金がかかる。
 *
 * 上限を掛ける道は残してある。欄は畳まずに出したままなので
 * （`index.html` の起こすフォーム）、入れたい人はその場で入れられるし、
 * 設定から既定そのものを変えられる（`run/defaults.mjs`）。
 * **金が減る話を欄ごと隠さない**、という線はそのまま守る。
 */
export const DEFAULT_BUDGET_USD = null;
