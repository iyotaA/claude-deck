...
**同じ紙に通知以外の設定も入る。** いまは起こしてよいフォルダ（`run.dirs`。`src/run/dirs.mjs`）。
読み書きは `shared/configfile.mjs` の1枚に寄せてあり、`mergeSettings` が知らないキーを残すので、
片方を保存してももう片方は消えない（テストで見ている）。
...
- **`stats()` を読み取りのままに保つ。** 呼んだだけで状態が変わってはいけない。前は中で `sendTimes.shift()` していて、`health()`（引数なし＝`Date.now()` で呼ぶ）が窓を空にしていた ―― 時刻を渡して確かめている最中に、別の呼び出しが状態を進めることになる。捨てるのは `takeReady`（状態を進める場所）だけ
- **1時間の上限（`MAX_PER_HOUR`）で止めたら、窓が空いたときに自分で戻す。** 前は `paused` から抜ける道が「起動し直す」しか無く、**忙しい日ほど上限に当たる**ので、いちばん通知が要る日に黙ることになっていた。`overLimit` を旗として持たず「その時刻で窓が埋まっているか」の判定にすれば、1時間が流れた時点で勝手に false へ戻る。止めるときは**残り時間も言う**（言わないと人は再起動しに行く）
