...
| `md.test.mjs` | Markdown のパーサ。記法を読めることと、途中で切れた入力で壊れないこと |
...
| 1 | `md-view.js` | パーサの結果を節点へ。口は `mdView(text, needle)` の1つだけ |
| 2 | `rows.js` | 一覧の行の導出 |
...
| `md-view.js` | ブロックの並び → 節点（`mdView`） | `document.createElement` だけ |

だから `md.js` は Node から import できて、`test/md.test.mjs` が 35 件通っている。
`parseUpdateState`（判断）と `loadUpdateState`（I/O）を分けたのと同じ形。
...
いま描いているのは4箇所。**頭出し（`tl-text`）はまだ素の文字のまま。**
あちらは文字数で切っているので、記法の途中で切れた断片を描くことになる。
切る単位をブロックへ移すまでは、素の文字のほうが読める。
...
- **`md.js` に DOM を触らせない。** あれが Node から import できて 35 件のテストに乗っているのは、`document` を1つも見ていないから。判断（`md.js`）と I/O（`md-view.js`）を混ぜると、画面側で唯一のテストが動かなくなる
...
- **頭出し（`tl-text`）を Markdown にしない。** あちらは文字数で切っているので、記法の途中で切れた断片を描くことになる。切る単位をブロックへ移すまでは素の文字のほうが読める
