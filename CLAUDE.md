...
| 場所 | 役割 | 中身 |
|---|---|---|
| `src/read/` | `~/.claude` を読む | `paths` `cache` `registry` `transcript` `tasks` `plans` `subagents` `skills`（索引） `grep`（本文を探す） |
...
| `src/run/` | 画面から起こすセッション | `index`（配線） `ledger`（台帳と状態機械） `ask`（許可要求カードの組み立て） `merge`（一覧への合流） `spec`（起動指定の検証と argv） `event`（速報1件の畳み方） `dirs`（起こしてよいフォルダの登録） `defaults`（起こすときの既定値） `rate`（枠の使用率を紙に1枚） |
...
| `GET /api/settings/rundefaults` | 起こすときの既定値。**選んである値だけ**（語彙・候補・範囲は `/api/runs/options` が持つ） |
...
| `POST /api/settings/rundefaults` | 起こすときの既定値を保存する。**触った項目だけ**送れる（キーが無い＝触らない）。応答は読み直した値なので、丸められたことが画面でその場で見える |
...
**誰も見ていなくて通知も要らないなら、ポーリングは 30 秒に落ちる**（`IDLE_POLL_MS`）。
このアプリは自動起動で常駐するので、ブラウザを1枚も開いていない時間のほうが長い。
`broadcast()` は `clients.size === 0` で即 return するが、それは**読み終わったあと**の話で、
`listSessions()`（登録簿の全読み ＋ 索引 ＋ 行ごとの `readTail`）はもう走っている。
実測で 60 秒あたり 30 回 → 2 回。**止めはしない**（`fs.watch` が効かない置き場所では
これだけが変化に気づく手段になる）。窓を開けているとき・通知が有効なとき・
子が走っているときは落とさない ―― とくに通知は「返事待ちに気づけない」を埋める仕組みで、
遅らせると鳴るのが遅れる。

**重い GET 3本（`/api/usage`・`/api/archive`・`/usage/baseline`）は合流する**（`coalesce`）。
同じ読み取りが重なったら走っているものに相乗りする。読み取り専用なので正しさは損なわれない。
タブを複数開いたときのほか、**外からの連打**にも効く（GET は書き込みの門番を通らないので、
他所のページから `fetch(mode:'no-cors')` で撃てる）。
...
- **切ったことは黙らない。** 上限で落としたぶんは数を返す（`/api/sessions` の `endedOmitted`、`/api/archive` と `/api/usage` の `scanLimited`、`/api/runs/options` の `modelsOmitted`）。**0 のときもキーを出す** ―― 消すと「切られたのか、この版が数えないのか」が区別できない
