...
`public/js/` は 29枚 ＋ `timeline/` 7枚。**import は上から下へ一方向にだけ流す。**
...
CSS は `public/css/` に14枚ある。`index.html` の `<link>` の並びが、そのまま重ね順になる。
...
`narrow.css` は上の13枚を上書きするので、必ず最後に読む。
...
| 「あなたの番」のプランと最後の応答 | `detail-wait.js` |
| 実行の速報の本文 | `run-view.js`（`bodyText` を借りている） |
