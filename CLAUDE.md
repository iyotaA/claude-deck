...
| 場所 | 役割 | 中身 |
|---|---|---|
| `public/css/` | 見た目 | 12枚。`<link>` の並びがそのまま重ね順になる |
| `public/js/` | 画面の組み立て | 25枚 ＋ `timeline/` 7枚 |
...
`public/js/` は 25枚 ＋ `timeline/` 7枚。**import は上から下へ一方向にだけ流す。**
...
誰からも import されない（呼ぶのは `main.js` だけ）。
...
- URL クエリで開き方を指定できる … `?session=<id>` `?theme=dark|light` `?only=1` `?nolive=1` `?tab=archive|usage` `?aq=` `?asort=` `?tq=` `?hide=`
...
CSS は `public/css/` に12枚ある。`index.html` の `<link>` の並びが、そのまま重ね順になる。
...
`narrow.css` は上の11枚を上書きするので、必ず最後に読む。
