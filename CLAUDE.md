...
| `public/js/` | 画面の組み立て | 26枚 ＋ `timeline/` 7枚 |
...
`public/js/` は 26枚 ＋ `timeline/` 7枚。**import は上から下へ一方向にだけ流す。**
...
- 画面のモードは `MODES`（`work` / `board`）で照合する。`TABS` と同じ扱いで、三項演算子で二値に畳まない。`localStorage` には残さない（監視盤を開いたまま保存すると、次に開いたとき作業台へ戻る道が「押す」しか無い状態で始まる）。固定したい人は `?mode=board` でブックマークする
...
- URL クエリで開き方を指定できる … `?session=<id>` `?theme=dark|light` `?only=1` `?nolive=1` `?tab=archive|usage` `?mode=board` `?aq=` `?asort=` `?tq=` `?hide=`
- 左のペインのタブは `TABS`（`live` / `archive` / `usage`）で照合する。三項演算子で二値に畳まない。畳むと知らない値がすべて `live` に落ちて、タブを1つ足すたびに2箇所（判定と `syncQuery`）を直すことになる
...
- **`narrow.css` に `.is-board .deck` の上書きを足す。** `board.css` だけでは足りない。あちらは `narrow.css` より前に読まれるので、同じ詳細度（0,2,0）の `.deck` に負けて、狭い画面だと列がレールの分だけ残る。レールごと `display: none` で消しているので、その列は幅ゼロのまま居座る
...
- **`usage.css` の `@media (min-width: 861px)` を外さない。** `.deck:has(.list-pane.is-usage)` は `:has()` を持つので、`narrow.css` の `.deck`（`@media` は詳細度を足さない）に素で勝つ。囲まないと、狭い画面で左を数値タブにした瞬間にレールの列が消える
